...
- **Copiar um dia para outros dias:** replica todas as Viagens do dia origem para um ou mais dias escolhidos, sempre dentro da mesma grade. Cada Viagem efetivamente copiada mantém `horario_saida` e `horarios_paradas` da origem, mas recebe UUID nova. A operação não remove nem atualiza Viagens já existentes no destino. Se o destino já tiver ao menos uma Viagem com o mesmo `horario_saida` na mesma grade, nenhuma Viagem da origem naquele horário é copiada para esse destino, inclusive eventuais reforços, e o usuário é avisado. O próprio dia origem não pode ser usado como destino.
...
- Botão **"Copiar dias comuns"**: caso particular da semeadura de grade definida em §8.5. Por padrão, usa a grade comum como origem e a tabela de feriados como destino; o seletor de origem também permite escolher a grade de feriados ou uma tabela excepcional do mesmo Serviço/sentido. Valem integralmente as regras de normalização e da ação **"Copiar (sobrescrever)"** definidas em §8.5.
...
4. **Itinerários por Serviço e sentido** — para cada Serviço e sentido, nesta ordem: (a) **título** `Serviço 0000-NXX — Ida` / `— Volta`;(b) **imagem do mapa** com a rota (captura do canvas — Spec 01 §8), seções numeradas e locais comuns. Locais comuns **não** aparecem na sequência de Seções, nem na descrição textual (só no anexo técnico). Regras de renderização da descrição: §13.4. (c) **sequência resumida de Seções** no padrão de lista, numerada na ordem das seções no itinerario e sentido em pauta;(d) **descrição textual do itinerário** por nomes de vias (`rota.descricao_itinerario.texto` — Spec 02 §10.5), ex.: `Cidade A - Seção A, Rua Treta, Avenida Santo Antônio, Cidade B - Seção B, Rodovia X, Cidade C - Seção C.`.
...
4. **Operações de cópia de Viagens** (§8.3–§8.5):
   - **"Copiar (sobrescrever)" entre grades:** sincroniza a grade destino com a origem escolhida. Viagens casadas por `horario_saida` preservam a UUID que já possuíam no destino e recebem os horários de passagem da origem; ausentes na origem são removidas; faltantes no destino são acrescentadas com UUID nova. UUIDs da origem nunca são reutilizadas. Em destino vazio, todas as Viagens copiadas recebem UUIDs novas.
   - **Copiar uma Viagem para outro dia:** cria uma Viagem com o mesmo `horario_saida` e os mesmos offsets, mas com UUID nova. Se o destino já tiver uma Viagem com o mesmo horário na mesma grade, nenhuma cópia é criada e a existente permanece inalterada.
   - **Copiar um dia para outros dias:** acrescenta aos dias escolhidos cópias das Viagens do dia origem, com UUIDs novas. Não remove nem atualiza Viagens existentes. Horários que já existam no destino e na mesma grade são ignorados.
