...
**Última atualização:** 2026-07-22 (branch `redesign`) — **TASK-064 concluída**: implementada em `d6a5fcf` (+ fixes `50f840b` e `7a3084f`) e **aprovada com ressalvas** em `14-REVISOES/TASK-064-20260722.md`; sincronização bidirecional tabela↔mapa com scroll-into-view, compondo com o erro da DEC-070. Antes no mesmo dia: criada a **TASK-091** (redesenho da tabela lateral unificada), dependente da nova **Q-052**. Antes (2026-07-21): TASK-079 implementada e aprovada; pontos de rota integram a lista lateral única, com reancoragem posicional e preservação do erro de Local extremo. TASK-064 fica desbloqueada.
...
74 tasks concluídas. Agrupadas pela fase do backlog.
...
| 091 | Redesenho da tabela lateral unificada (colunas, zebra, densidade, "X") — **bloqueada pela Q-052** |
...
| — | **091** — Redesenho da tabela lateral unificada (colunas, zebra, densidade, "X") | **3** | **Bloqueada pela Q-052** (composição zebra × erro × ciano × seleção; token global × variante). A **064 está entregue**: a 091 compõe **sobre** o realce de seleção existente (zebra/hover já cedem à linha `aria-current`, fix `7a3084f`) e herda o follow-up de teste da revisão da 064; não está na ordem "pronta" enquanto a Q-052 não for decidida. |
