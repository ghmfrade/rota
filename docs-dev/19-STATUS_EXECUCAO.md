...
| # | Task | Complex. | Observação |
|:---:|---|:---:|---|
| 1 | **070** — Clique sobre o vértice remove o ponto de rota | **2** | Independente de 067 (✅). **Dependência da TASK-097 cumprida** (entregue em `6b1360f`, 2026-07-24): com o arrasto perto da linha já não sendo revertido, um arrasto frustrado deixou de ser candidato a remoção destrutiva. |
...
| # | Task | Complex. | Observação |
|:---:|---|:---:|---|
| 2 | **076** — Ida e Volta no mesmo mapa (abas, tracejado, dois painéis) | **5** | A mais cara do backlog pendente. Assume a Volta derivada (077, entregue) com o disparo do espelho já corrigido (093, entregue); preserva erro de Local extremo por sentido, sem contaminar a ocorrência válida no outro (DEC-070). **TASK-097 entregue** (`6b1360f`, 2026-07-24): o invariante de arrasto está no lugar; a análise da 076 deve reafirmá-lo, já que abas e dois painéis multiplicam os re-renders da superfície do mapa e poderiam reintroduzir a reversão por outro caminho. |
| 3 | **078** — Realocação de Seção inteira (translação rígida) | **4** | Cascata de recálculo multi-Serviço — caminho novo. **Dependência dura da TASK-097 cumprida** (`6b1360f`, 2026-07-24): o gesto é um arrasto sobre o traçado, que já não é mais revertido. |
...
| 6 | **062** — E2E do fluxo "criar do zero" ponta a ponta | **3** | Depois do mapa estabilizado, senão os seletores mudam de novo. |
| 14 | **099** — Renomear `aoAtualizarSessao` → `aoDefinirSessao` (par homófono com `aoAtualizarSecao`) | **1** | **Prioridade baixa; roda por último deste grupo.** Refactor puro de nomenclatura, 59 ocorrências em 11 arquivos, sem mudança de comportamento. **Restrição dura de sequenciamento: depois de 097, 098, 070, 076 e 078** — todas editam `etapa-itinerarios.tsx` ou `shared/mapa/mapa.tsx`, e um rename atravessado nessa fila só gera conflito de merge. Nome final a confirmar pelo responsável (decisão de código, sem Q-xxx). |
...
| # | Task | Complex. | Observação |
|:---:|---|:---:|---|
| 7 | **033** — PDF operacional: estrutura e identificação | **4** | Subsistema novo (@react-pdf) + captura do mapa. |
| 8 | **034** — PDF operacional: tabelas horárias e matrizes | **3** | Sobre a 033. |
| 9 | **035** — Comparador: carregamento e validação dos dois arquivos | **3** | |
| 10 | **036** — Motor de diff por UUID + taxonomia | **5** | Núcleo do Comparador; casamento por UUID e por contexto. |
| 11 | **037** — Telas de comparação | **4** | Superfície ampla (5 visões/abas). |
| 12 | **038** — Mapa comparativo | **3** | Bloqueio parcial: Q-004 (tolerância). |
| 13 | **039** — PDF comparativo completo | **4** | |
