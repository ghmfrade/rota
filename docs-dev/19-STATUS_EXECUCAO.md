...
implementada** em `4051430` e recebeu parecer **aprovado com ressalvas** em
`14-REVISOES/TASK-101-20260727.md`. A reconciliação de `secao.servicos[]`
restaura as contribuições e os marcadores das Seções do Serviço copiado; a suíte
canônica está verde (1223 unitários + 73 E2E; fingerprint `592be101…`;
identidade `498d7fa6…`). O ciclo **ainda não foi marcado como concluído**:
permanecem como condições de merge um round-trip exportação→importação do
documento após duplicar e uma prova E2E explícita de zero chamadas ao OSRM.
Assim, a TASK-101 continua no Grupo A-ter e o total de pendentes permanece 9.
...
### Grupo A-ter — Bug vivo da duplicação de Serviço (2026-07-27)

Relatado manualmente pelo responsável, com causa-raiz localizada. **Prioridade máxima** (bug vivo que produz documento inválido e trabalho refeito à mão).

| # | Task | Complex. | Observação |
|:---:|---|:---:|---|
| 1 | **101** — Duplicar Serviço reconcilia `secao.servicos[]`: o novo Serviço contribui suas próprias geolocalizações às Seções compartilhadas | **2** | Bug: ao duplicar um Serviço, as Seções não aparecem no mapa (só na tabela/horários/matrizes) e o documento fica **inválido**. Causa: `duplicarServico` cunha `uuid` novo mas não acrescenta às Seções compartilhadas a entrada `secao.servicos[]` do novo `servico_uuid` (o mapa lê exatamente essa entrada — `editor-mapa-itinerario.tsx:152-156`; a validação a exige — RN-036/RN-026 em `validacoes-estruturais.ts:172-180`). Fecha a cláusula final da **RN-007**. Sem Q-xxx; sem campo novo de contrato. |

**Estado da TASK-101 em 2026-07-27:** implementada em `4051430` e aprovada com
ressalvas. O código central e o E2E de marcadores estão verdes, mas a task não
sai desta lista até cumprir as duas condições do parecer:
round-trip exportação→importação específico do documento duplicado e
instrumentação E2E que prove zero chamadas ao OSRM.
