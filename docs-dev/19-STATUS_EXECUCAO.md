...
**Última atualização:** 2026-07-22 (branch `redesign`) — criada a **TASK-091** (redesenho da tabela lateral unificada), dependente da nova **Q-052**; a sincronização da **TASK-064** passou a incluir o scroll-into-view da linha selecionada. Antes (2026-07-21): TASK-079 implementada e aprovada; pontos de rota integram a lista lateral única, com reancoragem posicional e preservação do erro de Local extremo. TASK-064 fica desbloqueada.
...
| 2 | **064** — Sincronização de seleção tabela↔mapa | **3** | Desbloqueada pela 079; seleção usa canal distinto e não mascara linha/borda vermelha de RN-035 (DEC-070). Inclui o scroll-into-view da linha selecionada (mapa→tabela). |
| — | **091** — Redesenho da tabela lateral unificada (colunas, zebra, densidade, "X") | **3** | **Bloqueada pela Q-052** (composição zebra × erro × ciano × seleção; token global × variante). Recomendada **depois** da 064, para compor sobre o realce de seleção; não está na ordem "pronta" enquanto a Q-052 não for decidida. |
