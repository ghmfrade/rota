...
| 14 | **099** — Renomear `aoAtualizarSessao` → `aoDefinirSessao` (par homófono com `aoAtualizarSecao`) | **1** | **Prioridade baixa; roda por último deste grupo.** Refactor puro de nomenclatura, 59 ocorrências em 11 arquivos, sem mudança de comportamento. **Restrição dura de sequenciamento: depois de 097 (entregue), 098 (entregue), 070, 076 e 078** — todas editam `etapa-itinerarios.tsx` ou `shared/mapa/mapa.tsx`, e um rename atravessado nessa fila só gera conflito de merge. Nome final a confirmar pelo responsável (decisão de código, sem Q-xxx). |
