...
76 tasks concluídas. Agrupadas pela fase do backlog.
...
A **TASK-077** saiu da lista: implementada em `553d1e6` e **aprovada com ressalvas** em `14-REVISOES/TASK-077-20260722.md` — espelhamento Ida↔Volta por gesto atômico de Seção (DEC-063/DEC-071). As três ressalvas do parecer foram resolvidas pela **DEC-074** (2026-07-23): a condição virou a **TASK-093** (abaixo, antes da 076); os follow-ups de aviso DEC-068 no sentido espelhado e de E2E de exportação foram **encerrados sem ação**; ver §3 e o cabeçalho deste documento.

| # | Task | Complex. | Observação |
|:---:|---|:---:|---|
| 2 | **093** — Suprimir o espelho no movimento de Seção que não altera a subsequência (DEC-074) | **1** | Fecha a condição do parecer da 077; **antes da 076**. Ponto único (`moverParada`) + teste de guarda. |
| 3 | **076** — Ida e Volta no mesmo mapa (abas, tracejado, dois painéis) | **5** | A mais cara do backlog pendente. Assume a Volta derivada (077, entregue); preserva erro de Local extremo por sentido, sem contaminar a ocorrência válida no outro (DEC-070). |
| 4 | **078** — Realocação de Seção inteira (translação rígida) | **4** | Cascata de recálculo multi-Serviço — caminho novo. |

### Grupo D — UX restante e qualidade

| # | Task | Complex. | Observação |
|:---:|---|:---:|---|
| 5 | **075** — Identificação: pré-visualização + confirmação explícita | **2** | Encaixe livre — independente de tudo acima. |
