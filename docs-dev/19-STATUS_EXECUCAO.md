...
78 tasks concluídas. Agrupadas pela fase do backlog.
...
|:---:|---|:---:|---|
| 1 | **078** — Realocação de Seção: botão esquerdo move o cluster, direito move o ponto corrente (DEC-061/**DEC-079**) | **4** | Cascata de recálculo multi-Serviço — caminho novo. **Dependência dura da TASK-097 cumprida** (`6b1360f`, 2026-07-24): o gesto é um arrasto sobre o traçado, que já não é mais revertido. **Redesenhada em 2026-07-24 (Q-057/DEC-079):** o mecanismo de "modo" (duplo clique) da DEC-061 foi substituído por atribuição direta de botão do mouse — sem dependência das TASK-070/076, canceladas. |
| 2 | **100** — Botão "redefinir Seção": todos os pontos voltam para o ponto do serviço/sentido em edição (**DEC-080**, nova) | **2** | **Criada em 2026-07-24** (Q-058/DEC-080). Reusa o motor de translação da TASK-078 (destino fixo em vez de vetor de arrasto) — roda **depois** dela. Botão de refresh na linha de Seção da tabela lateral, entre mover e remover; confirmação OK/Cancelar. |

### Grupo D — UX restante e qualidade

| # | Task | Complex. | Observação |
|:---:|---|:---:|---|
| 3 | **075** — Identificação: pré-visualização + confirmação explícita | **2** | Encaixe livre — independente de tudo acima. |
| 4 | **062** — E2E do fluxo "criar do zero" ponta a ponta | **3** | Depois do mapa estabilizado, senão os seletores mudam de novo. |
| 12 | **099** — Renomear `aoAtualizarSessao` → `aoDefinirSessao` (par homófono com `aoAtualizarSecao`) | **1** | **Prioridade baixa; roda por último deste grupo.** Refactor puro de nomenclatura, 59 ocorrências em 11 arquivos, sem mudança de comportamento. **Restrição dura de sequenciamento: depois de 097 (entregue), 098 (entregue), 078 e 100** — todas editam `etapa-itinerarios.tsx` ou `shared/mapa/mapa.tsx`, e um rename atravessado nessa fila só gera conflito de merge. (As TASK-070/076, que também tocavam esses arquivos, foram canceladas em 2026-07-24 e saem desta restrição.) Nome final a confirmar pelo responsável (decisão de código, sem Q-xxx). |
