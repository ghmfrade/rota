...
- **Tabela lateral de itinerários (DEC-073/TASK-091):** `Tabela densidade="compacta"` com quatro colunas explícitas (`Cidade - Nome` · Tipo · Mover · Remover); Tipo fica vazio para o ponto de rota (a natureza já é evidente pelo texto "Ponto de Rota N (lat, long)" na coluna de nome — RN-042); setas e "X" usam `Botao tamanho="compacto"` (variante `secundario` nas setas, `fantasma` no "X", com `aria-label`). Até quatro sinais coexistem na mesma linha por ocuparem canais físicos distintos: **fundo** = zebra em repouso, com a seleção da TASK-064 (`bg-azul-100`) sempre sobrepondo (via exclusão `:not([aria-current="true"])` na zebra/hover); **texto/ícone** = erro de Local extremo (`text-erro`, DEC-070) e ponto de rota (`text-ciano-500`, DEC-069); **borda** = erro de Local extremo e foco. Nenhum desses canais é reatribuído por outra tabela do app — a variante é local à prop `densidade`.
