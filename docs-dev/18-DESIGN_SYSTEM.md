...
|---|---|
| `Botao` | variantes `primario` (fundo `azul-600`, texto branco, `sombra-1`; hover `azul-700` + `sombra-2` + elevação `-translate-y-px`), `secundario` (borda `cinza-200`, fundo branco, texto `cinza-700`), `perigo` (fundo `erro`), `fantasma` (sem borda, hover `cinza-100`) e `alternador` (borda `azul-300`, fundo `azul-100`, texto `azul-900`; repouso de controle alternável). O alternador ativo usa `primario`, como no modo headway (DEC-093). Foco: anel `azul-300`. Desabilitado: opacidade + cursor. Prop `tamanho` (DEC-073/TASK-091): `padrao` (`px-4 py-2`, default) ou `compacto` (`px-2 py-1` — ações densas de tabela, ex.: "X" de remover e setas de mover). Nunca altera `type`/handlers recebidos. |
| `Campo` / `Select` | rótulo `text-xs` `cinza-500` acima; controle com borda `cinza-200`, `radius-controle`, foco com anel azul; erro com borda/texto `erro`. Encapsula `<input>`/`<select>` nativos repassando props (inclusive `data-testid`). Prop `densidade`: `padrao` (`px-3 py-2`, default) ou `compacta` (`px-2 py-1`, para controles em tabelas compactas). |
...
| `SuperficieFlutuante` | superfície **ancorada ao elemento pai** onde é declarada, com conteúdo arbitrário (inclusive formulário): fundo branco, borda `cinza-200`, `radius-controle` e `sombra-3`. Usa `position: fixed` com coordenadas medidas em runtime (exceção de `style=` do §6.1) para escapar do recorte de ancestrais com `overflow` — ex.: o wrapper de rolagem da `Tabela`. Prop `ladoPreferido` (`acima`/`abaixo`/`direita`/`esquerda`): tenta o lado pedido, inverte para o oposto quando não couber e, por fim, prende a caixa dentro da área útil (`limiteRef` ∩ viewport, folga de 8px), reposicionando-se em rolagem e redimensionamento. Nos lados verticais a caixa alinha-se pela **borda direita** da âncora, para não invadir a faixa lateral que outra superfície da mesma âncora ocupa (é o que mantém a coluna de ações da Viagem e a inserção posterior sempre clicáveis). Prop `larguraDaAncora` (DEC-102/TASK-124): a caixa assume a **largura da âncora**, tomada da mesma medição em runtime que já governa a posição, em vez da largura natural do conteúdo — é o que mantém as superfícies de ação da Viagem dentro da coluna do dia. O piso de legibilidade é o `min-content` do conteúdo (`min-w-min`): coluna mais estreita que o piso faz a superfície transbordar o mínimo indispensável, nunca truncar o conteúdo nem encolher o alvo do botão — o chamador precisa compor com faixas de grid `minmax(min-content,1fr)` (não `minmax(0,1fr)`, que zera a contribuição do campo ao `min-content` do contêiner). Sem a prop (default), a largura continua vindo do conteúdo — é o caso da coluna de ações da Viagem. Prop `empilhamento`: `padrao` (`z-40`) ou `prioritaria` (`z-50`) — quando o reposicionamento em borda ainda assim sobrepõe duas superfícies da mesma âncora, a prioritária fica por cima e continua clicável. Prop `aberta` é governada pelo chamador: **não** rouba foco ao abrir, **não** cria backdrop e **não** fecha por clique fora — ao contrário do `MenuFlutuante`, que é um `role="menu"` de opções e continua sendo o componente certo para escolha discreta. Fechada, permanece montada, inerte e transparente, para não recriar estado a cada hover. |
